---
layout: post
title:  "[멋사 백엔드 19기] TIL 46일차 Rest API 2"
...
## 📂 목차
- [URI 설계 원칙](#uri-설계-원칙)
- [HTTP 상태 코드의 세분화](#http-상태-코드의-세분화)
    - [RestControllerAdvice](#http-상태-코드의-세분화)
- [Versioning](#versioning)
- [Content Negotiation](#content-negotiation)
- [ETag 기반 조건부 요청 (If-Match, If-None-Match)](#etag-기반-조건부-요청-if-match-if-none-match)
...
- [CORS & CSRF](#cors--csrf)
    ...
    - [CSRF](#csrf)
        - [CSRF 토큰](#csrf-토큰)
        - [SameSite Cookie 설정](#samesite-cookie-설정)
        - [Referer / Origin 검증](#referer--origin-검증)
- [Spring HATEOAS 심화](#spring-hateoas-심화)
...
2. `ReturnHandler` 의 `HandlerMethodReturnValueHandler` 로 `Accept` 헤더 읽어들임
...
CORS(Cross-Origin Resource Sharing) 는 **브라우저에서 다른 출처(Origin) 으로의 요청을 보낼 때 발생하는 보안 정책을 제어하는 기법**이다.
...
이 중 `OPTIONS 요청`이 `Preflight` 요청의 핵심이며, 목적은 브라우저가 실제 요청을 보내기 전에 서버가 허용하는 메서드, 헤더, 출처를 미리 확인하는 용도로 사용하게 된다. 항상 `OPTIONS` 요청이 먼저 일어나고 그 다음 `PUT`, `DELETE`, `PATCH` 요청-응답이 이루어진다.
