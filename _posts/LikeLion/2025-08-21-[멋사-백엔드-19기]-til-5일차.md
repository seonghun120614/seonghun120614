...
우리가 서비스 할, 목표로 하는 목적에 맞게 현실 세계에서의 문제를 정의하고, 그 이후에 클래스를 설계한다.

클래스 선언에서 맨 위는 필드가 보이도록 하며, 그 밑에 메서드가 나온다.

#### Static

`static` 은 특별한 키워드이다. **정적**의 의미를 가지며, 정적 이라는 소리는 동적과는 반대되는 의미이다. 동적이라는 것은 움직이는, 변하는의 의미를 가진다. 정적은 변하지 않는 이라는 의미를 가진다.

따라서 정적이라는 것은 동적과는 다르게 변하지 않을 것이며, 항상 유일하게 하나로 존재할 것이다. 그것이 **JVM 이 loading 될 때의 그 세상에 하나 메모리에 하나의 의미를 지니게** 될 것이다.

> 더 깊이있게 나아가자면, JVM 이 올라가고 나서 Method Area 에 로드되는 정적 영역에 위치하게 된다.

##### Static Field

필드는 클래스가 가지는 변수이다. 이 변수가 변하지 않는다는 것, 유일하다는 것이다.

이런 변수들은 `class` 가 `public` 하고 `field` 가 `public static` 이면 다른 객체가 변경을 할 수 있게 된다.

이미 메모리에 올라와져 있는 이 `static` 키워드가 붙은 field 는 굳이 `instance initializing` 을 하지 않아도 접근이 가능하다.
...
클래스는 무조건 `Exception` 을 띄운다. 이때 JVM이 Bed 클래스를 처음 로드 & 초기화를 할 때 static 블록이 실행되는데,
...
이제 이 클래스는 초기화 실패한 클래스로 JVM에 표시되어 있고, 이를 `new Bed()` 를 하거나 `Bed.(static 메서드)()` 를 하면 `NoClassDefFoundError` 가 발생하게 된다.
...
public class Main{
    public static void main(String[] args) {
        ...
        Bed.hello(); // NoClassDefFoundError 발생
    }
}
```
...
이것 또한 `JVM` 을 심도 있게 안다면 바로 알 수 있는 내용인데 `static` 자체는 `JVM` 이 올라가고 나서 `Method Area` 영역에 로드 되는 `Static Area` 에 위치하게 된다. 

**따라서 `Static Nested Class` 는 바깥 클래스의 인스턴스에 종속된게 아니다.** 그 자체로 `JVM` 이 클래스 로딩을 끝내는 순간 메서드 영역에 올라가버린다. 이 말은 바깥 클래스의 객체 상태(`this`)와는 독립적으로 존재하기 때문에, 바깥 클래스의 인스턴스 필드에 접근할 수 없고 오직 정적 멤버만 참조 가능하다는 것이다. <- 이거는 `public static void main` 에서 `static` 이 아닌 method 를 불러올 때도 마찬가지로 에러가 뜸을 볼 수 있는 것과 동일한 원리다.

만약 `Static Nested Class` 가 설계된 `.class` 파일을 `javac` 로 컴파일 하게 되면 `Outer$StaticNestedClass.class` 와 같은 별토의 클래스 파일이 생성됨을 볼 수 있을 것이다(중요).
...
Bootstrap 이 찾지 못한 클래스들을 로딩하며, JDK 확장 라이브러리(lib/ext 또는 모듈)을 로드한다. 예를들면 javax.* 가 있겠다.
